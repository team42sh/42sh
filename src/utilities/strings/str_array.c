 ...
 */
static void is_counted_as_new_word(char const *string, bool *in_quotes,
    int *amount, int *i)
{
    ...
    if (!(*in_quotes) && !is_input_delimiter(string[*i]) &&
        ...
        || is_quote_delimiter(string[*i - 1], *i > 1 ? string[*i - 2] : 0))) {
        (*amount)++;
    }
}
 ...
 * - word
 */
static int get_word_length(const char *str)
{
    int len = 0;
    ...
    while (str[len] && !is_input_delimiter(str[len]) && str[len] != '"'
        && len < MAX_WORD_LEN)
        len++;
    return len;
}
 ...
 */
static char *extract_word(char const *src, int len)
{
 ...
 */
static int skip_delimiters(const char *str, int index)
{
 ...
 */
static char **populate_word_array(char const *str, char **words, int word_len)
{
    int word_i = 0;
    int temp_word_len = 0;
 ...
 * TODO: Handle ' with "
 */
char **my_strarray(char const *str)
{
    int word_count = 0;
    char **words = NULL;
